...
export function CreateThematicProjectDialog({
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const { error } = await supabase.from('thematic_projects').insert({
        title: data.title,
      ...
      toast.success('Projeto Temático criado com sucesso!');
      form.reset();
      onOpenChange(false);
      onSuccess();
    } catch (error) {
      console.error('Error creating thematic project:', error);
      toast.error('Erro ao criar Projeto Temático');
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Novo Projeto Temático</DialogTitle>
          <DialogDescription>
            Crie um novo projeto temático para agrupar subprojetos de um mesmo financiador.
          </DialogDescription>
        </DialogHeader>
